...
namespace TeamGame
{
    ...
    {
        public static void HandleCollisions(List<MovingObject> movingObjects, 
        ...
        private static int DiagonalCollisionIndex(MovingObject moving, 
            List<GameObject> objects)
        {
            List<MatrixCoords> profile = moving.GetCollisionProfile();

            List<MatrixCoords> diagonalProfile = new List<MatrixCoords>();

            foreach (var coord in profile)
            {
                diagonalProfile.Add(new MatrixCoords(coord.Row + moving.Speed.Row, coord.Col + moving.Speed.Col));
            }

            int collisionIndex = GetCollisionIndex(moving, objects, diagonalProfile);

            return collisionIndex;
        }

        private static int HorizontalCollisionIndex(MovingObject moving, 
            List<GameObject> objects)
        {
            List<MatrixCoords> profile = moving.GetCollisionProfile();
        ...
        private static int VerticalCollisionIndex(MovingObject moving,
            List<GameObject> objects)
        {
            List<MatrixCoords> profile = moving.GetCollisionProfile();
        ...
        private static int GetCollisionIndex(MovingObject moving, 
            ICollection<GameObject> objects, List<MatrixCoords> movingProfile)
        {
            int collisionIndex = 0;

            foreach (var obj in objects)
            {
                if (moving.CanCollideWith(obj.GetCollisionGroupString())
                    || obj.CanCollideWith(moving.GetCollisionGroupString()))
                {
                    List<MatrixCoords> objProfile = obj.GetCollisionProfile();

                    if (ProfileIntersect(movingProfile, objProfile))
                    {
                        return collisionIndex;
                    }
                }
        ...
        private static bool ProfileIntersect(List<MatrixCoords> firstMovingProfile,
            List<MatrixCoords> secondObjProfile)
        {
            foreach (var firstCoord in firstMovingProfile)
            {
                foreach (var secondCoord in secondObjProfile)
                {
